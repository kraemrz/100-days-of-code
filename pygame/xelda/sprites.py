...
class NPC(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        self.groups = game.all_sprites, game.NPC
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        ...
        self.speed = choice(NPC_SPEED)

    def avoid_npc(self):
        for npc in self.game.NPC:
            if npc != self:
    ...
    def update(self):
        ...
        self.image = pg.transform.rotate(self.game.NPC_img, self.rot)
        self.rect = self.image.get_rect()
        self.rect.center = self.pos
    ...
    def draw_health(self):
        if self.health > 60:
            ...
            col = RED
        width = int(self.rect.width * self.health / NPC_HEALTH)
        self.health_bar = pg.Rect(0, 0, width, 7)
        if self.health < NPC_HEALTH:
